...
n_perm <- 15e3
n_marker <- 150 # Number of rvs/set
ncov <- 4
...
get_pvals <- function(ped1, pheno, mu_y, delt, GCV_mat){
	...
	genos <- gendrop(ped1[,.(fa,mo)], .2, n_marker)
	num <- crossprod(genos, ytot)
	denum <- outer(colSums(genos^2), varsY)
	z <- abs(num)/sqrt(denum)
	
	sapply(1:n_marker, function(i){
		eq <- sum( abs(z[i,-1]-z[i,1]) < tol )
		num_pval <- sum((z[i,-1] - z[i,1]) > tol)+ sum(runif(eq) > .5)
		(num_pval + 1.0)/ ncol(z)
	})	
}
...
for(i in 1:n_M) {
	res[[i]] = mclapply(1:n_rep, runme, mc.cores = 5, Tu= M$Tu[i], Tot = M$Tot[i])
