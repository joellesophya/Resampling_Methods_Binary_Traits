...
void VT(struct FAMILY *family, struct RV_INFO rv, int i_set, struct DATA_STRUCT data_struct, struct STATS *stats,
	double *MAF, double *sorted_MAF, int *unique_MAF, double *threshold, double *z, int perm){

	int i, j, fam, m, uni, T, g_ij, tmp_d, founder_allele;
	...
	// Step 1: Compute MAF for each variant
	sum_trait = 0.0;
	threshold[0] = 0;
	...
	for (T = 2; T <= uni; T++){
			...
			if ((MAF[i] >= threshold[T - 1]) && (MAF[i] < threshold[T])){
				...
				for (fam = 1; fam <= n_fam; fam++){
					...
					for (j = 1; j <= n_person; j++){
						...
						tmp_denum += tmp_d*tmp_d*1.0;
						tmp = tmp_d*(trait[j] * 1.0 - mean_trait);
						tmp_num += tmp;
					}
				}
				cumsum_num += tmp_num;
		...
		z[T - 2] = cumsum_num / sqrt(cumsum_denum);
	}
