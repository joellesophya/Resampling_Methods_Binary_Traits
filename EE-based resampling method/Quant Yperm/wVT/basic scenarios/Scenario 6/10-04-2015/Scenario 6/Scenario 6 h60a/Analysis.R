# Binary Y no cov &asc with VT
# sibship correlation is 3%
n_rv = 150
setwd("C:/Users/JOELLE/Google Drive/Reasearch Mary Sara _ SAMSUNG//C Code//Project/Permutation based methods/Binary trait")

p_vals4=read.table('Results/10-04-2015/VT - Scenario 6a h60a/pvals_20000.txt',header=F)
p_vals5=read.table('Results/10-04-2015/VT - Scenario 6a h60b/pvals_20000.txt',header=F)
p_vals6=read.table('Results/10-04-2015/VT - Scenario 6a h60c/pvals_20000.txt',header=F)
p_vals4=unlist(c(p_vals4));names(p_vals4)=c()
p_vals5=unlist(c(p_vals5));names(p_vals5)=c()
p_vals6=unlist(c(p_vals6));names(p_vals6)=c()
p_vals=c(p_vals4,p_vals5,p_vals6); rm(p_vals4,p_vals5,p_vals6)

########################### For p_vals from permutation method
summary(p_vals)
n=length(p_vals);n/n_rv
hist(p_vals, main="Scenario 6 - with Variable Threshold",freq = F,breaks=100, xlab="p value")
title(expression(paste("Sibship correlation = 3%")),line=.7)
abline(h=1,col="red")
