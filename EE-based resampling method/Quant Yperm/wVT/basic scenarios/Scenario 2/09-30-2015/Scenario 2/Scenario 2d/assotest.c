...
void VT(struct FAMILY *family, struct RV_INFO rv, int i_set, struct DATA_STRUCT data_struct, struct PHI_SVD phi_svd, struct STATS *stats,
	...
	// Step 1b: Get C^t*A where A=[G1^tT^{1/2},...,Gm^tT^{1/2},T^{-1/2}Y-mu_hat] for each family where T = diag{mu(1-mu)}
	...
	for (fam = 1; fam <= n_fam; fam++){
		...
		for (j = 1; j <= n_person; j++){
			for (i = 1; i <= n_marker; i++){
				...
				GY[fam][j][i] = tmp * sqrt(family[fam].mu_hat[j] * (1 - family[fam].mu_hat[j]));
			}
			GY[fam][j][n_marker + 1] = (trait[j] - family[fam].mu_hat[j]) / sqrt(family[fam].mu_hat[j] * (1 - family[fam].mu_hat[j]));
		}
	}
	...
	for (T = 2; T <= uni; T++){
		...
		z[T - 2] = fabs(cumsum_num) / sqrt(cumsum_denum);
	}
