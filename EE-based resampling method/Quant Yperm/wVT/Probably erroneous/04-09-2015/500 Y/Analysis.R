...
p_vals=read.table('Results/From April 9 2015/EE - RV - 500 Y/pvals_15000.txt',header=F)
...
alpha=c(.005,.01,.05)
for(a in alpha){
  pv=prop.test(x = sum(p_vals<a),n2,p = a)$'p.val'
  error1=as.numeric(prop.test(x = sum(p_vals<a),n2,p = a)$'est')
  #prop.test(x = sum(p_vals<alpha),n2,p = a)$'conf.int'
  
  print(c("alpha=",a), quote=F)
  print(c("type 1 error rate is",error1),quote=F)
  print(c("p-value is",pv),quote=F)
  print(c("SE:",round(sqrt(mean(p_vals<a)*(1-mean(p_vals<a))/n2),4)),quote=F)
  print("--------",quote=F)
}
