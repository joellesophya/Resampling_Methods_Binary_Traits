...
void permute(struct FAMILY *family, struct DATA_STRUCT data_struct, struct TMP_PERM tmp_perm) {
	...
	for (fam = 1, tot = 1; fam <= n_fam; fam++){
		for (i = 1; i <= n_person; i++, tot++) tmp_perm.v1Delta_f[fam][i] = tmp_perm.v1Delta[tot];
	}
	...
	for (fam = 1; fam <= n_fam; fam++){
		...
		for (i = 1; i <= n_person; i++, g++){
			for (j = 1, y_perm[i] = 0; j <= n_person; j++) y_perm[i] += sqrt(mu[i] * (1 - mu[i])) * chol[j][i] * tmp_perm.v1Delta_f[fam][j];
			y_perm[i] += mu[i];
	...
	for (fam = 1; fam <= n_fam; fam++){
		for (i = 1; i <= n_person; i++){
			if (family[fam].y_perm[i]>= thresh) family[fam].y_perm[i] = 1;
			else family[fam].y_perm[i] = 0;
		}
	}
	...
	for (k = 1; k <= n_cov; k++) {
		for (i = 1, wd[k] = 0; i <= n_fam; i++) {
			for (j = 1; j <= n_person; j++) wd[k] += family[i].cov[j][k] * family[i].y_perm[j];							
		}
	}
	...
	for (i = 1; i <= n_fam; i++) {		
		for (j = 1; j <= n_person; j++) {
			for (k = 1, resid[i][j] = family[i].y_perm[j]; k <= n_cov; k++) resid[i][j] -= family[i].cov[j][k] * bhat[k];
	...
	printf("%f %f\t", tmp_hat, 315.0);
	printf("%f %f\n", mu2_y - tmp_hat*tmp_hat / 990.0, 315 - (315 * 315 / 990.0));
	exit(1);*/
	
	// FILE *pfile;
	// pfile=fopen("y_replicate.txt","w");
	// for (fam = 1; fam <= n_fam; fam++){
		// for (i = 1; i <= n_person; i++){
			// fprintf(pfile, "%f\n", family[fam].y_perm[i]);
		// }
