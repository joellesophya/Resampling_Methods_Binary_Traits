// Not using marker replicates to compute the p-value
...
int main(int argc, char **argv) {
	...
	for (i_rep = 1; i_rep <= n_rep; i_rep++){
		...
		for (i = 1; i <= n_set; i++){
			VT(family, rv, i, data_struct, phi_svd, &stats, MAF, sorted_MAF, unique_MAF, threshold, z, 0);
			VT_obs[i] = stats.vt;
		}
		...
		for (i_perm = 1; i_perm <= n_perm; i_perm++){
			...
			for (i = 1; i <= n_set; i++){
				VT(family, rv, i, data_struct, phi_svd, &stats, MAF, sorted_MAF, unique_MAF, threshold, z, 1);
				if (stats.vt > VT_obs[i]) p_vals[i] += 1;
				else if (stats.vt == VT_obs[i]) p_vals[i] += (rand1(&seed) < .5) ? 1 : 0;
			}
