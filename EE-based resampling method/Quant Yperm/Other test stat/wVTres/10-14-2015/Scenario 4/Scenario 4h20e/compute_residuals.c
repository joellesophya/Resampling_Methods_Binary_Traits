...
double compute_gee_res(struct FAMILY *family, struct PHI_SVD phi_svd, struct DATA_STRUCT data_struct){
	...
	mle_file = fopen("mle_est_param.txt", "a");
	for (i = 1; i < n_cov; i++) fprintf(mle_file, "%f ", optim_betahat[i]);
	fprintf(mle_file, "%f\n", optim_betahat[n_cov]);
	fclose(mle_file);
	...
	// X^tY stored inside beta_init
	
	for (i = 1; i <= n_cov; i++) {
		for (j_cov = 1, beta_init[i] = 0; j_cov <= n_cov; j_cov++){
			ww[i][j_cov] = 0.0;		
		}			
	}
	...
	for (i = 1; i <= n_fam; i++) {
		for (j = 1; j <= n_person; j++) {
			for (k = 1; k <= n_cov; k++) {
				beta_init[k] += family[i].cov[j][k] * family[i].trait[j];
				for (j_cov = 1; j_cov <= n_cov; j_cov++) {
					ww[k][j_cov] += family[i].cov[j][k] * family[i].cov[j][j_cov];
				}
			}
		}
	}
	...
	for (i_cov = 1; i_cov <= n_cov; i_cov++) {
		...
		aug[i_cov][i_cov] = 1.0;
		aug[i_cov][n_cov + 1] = beta_init[i_cov];
	}
	// Get (X^tX)^(-1)
	int posdef = 1;
	...
	for (i_cov = 1; i_cov <= n_cov; i_cov++) {		
		for (j_cov = 1, bhat[i_cov] = 0; j_cov <= n_cov; j_cov++) {
			bhat[i_cov] += aug[j_cov][i_cov] * aug[j_cov][n_cov + 1];
			for (k = 1, xtxI[i_cov][j_cov] = 0; k <= n_cov; k++) {
				xtxI[i_cov][j_cov] += aug[k][i_cov] * aug[k][j_cov];
			}
		}
	}
	...
	for (i = 1; i <= n_fam; i++) {		
		for (j = 1; j <= n_person; j++) {
			for (i_cov = 1, resid[i][j] = family[i].trait[j]; i_cov <= n_cov; i_cov++) resid[i][j] -= family[i].cov[j][i_cov] * bhat[i_cov];
			tmp_e += resid[i][j] * resid[i][j];
		}
	}
	bhat[n_cov + 1] = tmp_e / (n_total- n_cov) ;
		
	// // Check that solution satisfies the EE
	// double tmp = 0;
	// double **chol = dmatrix(1, n_person, 1, n_person + n_fam);
	// double **C = dmatrix(1, n_person, 1, n_person);
	// for (i = 1; i <= n_person; i++) {		
		// for (j = 1; j <= n_person; j++) chol[i][j] = (i==j)? 1 : 0;
		// for (fam = 1; fam <= n_fam; fam++) {
			// chol[i][n_person + fam] = family[fam].trait[i] - family[fam].mu_hat[i];
		// }
	// }
	
	// k = cholesky(sig, n_person, chol, n_person + n_fam, C, chol, 1);
	// for (fam = 1; fam <= n_fam; fam++) {
		// for (j = 1; j <= n_person; j++) {
			// for (k = 1; k <= n_person; k++) {
				// tmp += chol[k][j] * chol[k][n_person + fam];
			// }
		// }
	// }
	// printf("%f\n", tmp);
	// free_dmatrix(chol, 1, n_person, 1, n_person + n_fam);
	// free_dmatrix(C, 1, n_person, 1, n_person);
	// exit(1);
