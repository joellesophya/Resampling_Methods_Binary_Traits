...
int main(int argc, char **argv) {
	...
	for (i_rep = 1; i_rep <= n_rep; i_rep++){
		...
		if (i_rep == 1){
			...
			data_struct.xtxInv = dmatrix(1, n_cov, 1, n_cov);
			data_struct.beta_hat = dvector(1, n_cov + 1);
			data_struct.res_Y = dmatrix(1, n_fam, 1, family[1].n_person);
			
			// For VT statistic
			stats.vt = 0;
			stats.vt_obs = dvector(1, n_set);
			...
			tmp_perm.delta = dvector(1, data_struct.n_total - n_cov);
			tmp_perm.delta_perm = dvector(1, data_struct.n_total - n_cov);
			tmp_perm.v1 = dmatrix(1, data_struct.n_total, 1, data_struct.n_total - n_cov);
		...
		////////////////////////////////////////////////////////
		// Check that solutions for paramters satisfy the estimating equations
		// for different values of the additive variance parameter
		compute_gee_res(family, phi_svd, data_struct);
		
		// Get VT stat for each set of RV sites for Y realization
		for (i = 1; i <= n_set; i++)	VT(family, rv, i, data_struct, phi_svd, stats, 0);
		...
		for (i_perm = 1; i_perm <= n_perm; i_perm++){
			...
			for (i = 1; i <= n_set; i++){
				VT(family, rv, i, data_struct, phi_svd, stats, 1);
				if (stats.z[1] > stats.vt_obs[i]) p_vals[i] += 1;
				else if (stats.z[1] == stats.vt_obs[i]) p_vals[i] += (rand1(&seed) < .5) ? 1 : 0;				
			}
	...
	free_dmatrix(data_struct.xtxInv, 1, n_cov, 1, n_cov);
	free_dvector(data_struct.beta_hat, 1, n_cov + 1);
	free_dmatrix(data_struct.res_Y, 1, n_fam, 1, family[1].n_person);
	...
	free_dvector(stats.vt_obs, 1, n_set);
	...
	free_dvector(tmp_perm.delta, 1, data_struct.n_total - n_cov);
	free_dvector(tmp_perm.delta_perm, 1, data_struct.n_total - n_cov);
	free_dmatrix(tmp_perm.v1, 1, data_struct.n_total, 1, data_struct.n_total - n_cov);
