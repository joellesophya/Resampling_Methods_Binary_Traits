...
SEXP VT_analysis(SEXP Yvec, SEXP agevec, SEXP sexvec, SEXP Zvec, SEXP varval, SEXP betavec) {
	...
	int i, j, k;
	int n_fam, n_person, n_founder_total, n_cov, n_set, n_cases_fam, n_cases_fam_obs;
	...
	// Simulate the replicates with conditioning on number of cases in each family
	i_perm = 1, k = 0;
	while( (i_perm <= n_perm) && (k <= (n_perm/.001*n_fam)) ){
		j = 1, data_struct.n_cases[i_perm + 1] = 0;
		while (j <= n_fam){
			run_trait_sim(family[j], stats.mles, n_cov, i_perm, &seed);	
			for (k++, n_cases_fam = 0, n_cases_fam_obs = 0, i = 1; i <= n_person; i++){
				n_cases_fam_obs += family[j].trait[1][i];
				n_cases_fam += family[j].trait[i_perm + 1][i];
			}
			if (n_cases_fam  == n_cases_fam_obs){
				data_struct.n_cases[i_perm + 1] += n_cases_fam, j++;
			}
		}
		++i_perm;
	}
	...
	if ( i_perm <= n_perm ){
		for (i = 0; i < n_set; i++) REAL(pvals_out)[i] = -1;
	}
	else{
		// Simulate genotype
