...
########################### 
################## Analysis
plot_power <- function(power_mat, props, meth_list, thr){
sds <- 2*sqrt(.25 / n_p)
n_method <- length(meth_list)
col_vec <- rainbow(n_method)
pch_vec <- 14 + seq(n_method)

pdf(paste0("power_plot_",ifelse(thr==0,"all",thr),".pdf"))
...
draw_pval <- function(ind, p_vals_list, meth_list){
...
rm(list= ls()[!(ls() %in% c('res','M','n_rep'))]); save.image()
...
library(Hmisc)
Tu_vals <- props/100
plot_power(na.omit(power_mat), Tu_vals, method_used, 0)
...
n_meth <- length( (method_used = c('MCMCglmm', 'GMMAT','CARAT','CERAMIC')) )
power_mat <- matrix(NA, n_M, n_meth, dimnames = list(paste(Tu_vals*100, (1-Tu_vals)*100, sep = "/"), method_used))
type1err <- matrix(NA, n_M, n_meth, dimnames = list(paste(Tu_vals*100, (1-Tu_vals)*100, sep = "/"), method_used))
n_pvals <- NULL

for(k in 1:n_M){
assign(paste0('p_vals',k,'_t1err'), vector("list", n_meth)) 
assign(paste0('p_vals',k,'_pow'), vector("list", n_meth)) 
}
...
library(Hmisc)
plot_power(power_mat, Tu_vals, method_used)
...
par(mfrow = c(2,2))
for(k in 1:n_M){
	eval(parse(text=paste0('lapply(1:n_meth, draw_pval, p_vals',k,'_t1err, method_used)')))
