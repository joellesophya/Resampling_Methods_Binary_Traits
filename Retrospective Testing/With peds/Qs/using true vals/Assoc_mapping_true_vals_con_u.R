...
runme = function (dummy, Tu, Tot)
{
	...
	int <- qlogis(0.025) - betas[1] * 31.86 -  betas[2] * 1.5 - LG * mean_G1G2  # Prevalence - b * E(X) - lambda * mean of epistatis indicator
	pvals_t1err = NA; pvals_power = NA
	
	while(any(is.na(pvals_t1err)) | any(is.na(pvals_power))){
		Y <- NULL; age <- NULL; Z <- NULL; G1 <- NULL ; Utot <- NULL
		# G2 <- NULL  
		...
		for (i_fam in 1:N_fam){
			...
			while(ascert == 0){
				...
				Y_f <- rbinom(n_person, size = 1, prob = plogis(int + U + LG * ((G1_f > 0) & (G2_f > 0)) +	age_f * betas[1] + sex[ped1$fam == i_fam] * betas[2] + Z_f * betas[3]))
		...
		fam_data = data.frame(Y=Y, age = age, sex = sex, Z = Z, U = Utot, idU = 1:n_total)

### MCMC glmm
		pi_hat <- plogis(model.matrix(~ Utot + age + sex + Z , data = fam_data) %*% c(int,1, betas))
...
M = rbind(c(0,5.2),c(0.4,6),c(.8,6.3),c(1,6.4))
n_M = dim(M)[1]
