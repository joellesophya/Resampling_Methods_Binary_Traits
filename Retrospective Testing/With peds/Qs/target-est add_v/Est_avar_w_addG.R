...
runme = function (dummy, Tu, Tot)
{
	...
	beta <- beta * S_XB
	LG <- log(1.5) # increase of 10% in the odds of being a case
	int <- qlogis(0.08) - beta[1] * 31.86 -  beta[2] * 1.5 - LG * mean_G1  # Prevalence - b * E(X) - lambda * mean of G1
...
### MCMC glmm
	...
	est_var1 <- posterior.mode(mMCMC$VC[,1]) / (1 + ck)
	est_var2 <- mean(mMCMC$VC[,1]) / (1 + ck)
	
	Ygu <- U_full + LG * G1
	target_var <- summary(lm(crossprod(diag(N_fam) %x% C_inv,Ygu)~crossprod(diag(N_fam) %x% C_inv,rep(1,n_total))-1))$sig^2
	
	pvalstot <- list(target = target_var, post_mode = est_var1, post_mean = est_var2, effectiveSize = effS)
    return(pvalstot)
}

M = rbind(c(0.1,5.2),c(0.5,6.2))
n_M = dim(M)[1]
...
sapply(seq(n), function(i){ cond1 <- effS[[i]]>50; cor(target_v[[i]][cond1], est_v_mean[[i]][cond1])})
sapply(seq(n), function(i){ cond1 <- effS[[i]]>50; cor(target_v[[i]][cond1], est_v_mode[[i]][cond1])})

head(cbind(est_v_mean[[2]],est_v_mode[[2]], effS[[2]], target_v[[2]])[order(effS[[2]]),],50)
tail(cbind(est_v_mean[[2]],est_v_mode[[2]], effS[[2]], target_v[[2]])[order(effS[[2]]),],15)

# Take lincomb of mean/mode as estimate???