#No ascert + Additive causal G -- u_hat = post mean of u
library(mvtnorm)
library(parallel)
library(reshape)
...
chol_phi <- chol(phi); C_inv <- solve(chol_phi); bigC_inv <- diag(N_fam) %x% C_inv
...
runme = function (dummy, Tu, Tot)
{
	...
	beta <- beta * S_XB
	LG <- log(3) # increase of 10% in the odds of being a case
	int <- qlogis(0.05) - beta[1] * 31.86 -  beta[2] * 1.5 - LG * mean_G1  # Prevalence - b * E(X) - lambda * mean of G1
...
### MCMC glmm
		...
		fam_data$U_hat <- colMeans(mMCMC$Sol[,-(1:mMCMC$Fixed$nfl)])
		
## Generating unassociated Gs
		n_G <- 50 # 50 * n_rep replicates for type 1 error
		Gmat <-  cbind(G1, sapply(1:n_G, function(a) rowSums(peddrop( ped1[,c("fa","mo")], 0.3 )))) # G1 is first column + n * n_G matrix 
		
		numend <- paste0("_",sample(1e6,1),"_")
		filend <- paste0(numend, '.txt')
		
## Using MASTOR-GLS to model u against G + g
		res_fam <- c(crossprod(H_mat, matrix(crossprod(Prec, fam_data$U_hat), n_person, N_fam)))
		T_num <- colSums(Gmat * res_fam)^2
		g_bar <- colSums( ((diag(N_fam) %x% crossprod(C_inv, H_mat)) %*% Gmat)^2 ) / (n_total - 1)
		T_denum <- sum(fam_data$U_hat * res_fam) * g_bar
		pvalstot <- pchisq(T_num / T_denum, df = 1, lower.tail = F)
		pvals_t1err <- pvalstot[-1]
		pvals_power <- pvalstot[1]
	
		pvals_power <- append(pvals_power, summary(lm(crossprod(bigC_inv, fam_data$U_hat)~crossprod(bigC_inv, cbind(1, G1))-1))$coe[4])
# # GMMAT
...
M = rbind(c(0.1,5.2),c(0.3,6.1),c(0.5,6.2),c(.8,6.3),c(1,6.3))
n_M = dim(M)[1]
