...
Prec <- MCMCglmm::inverseA(ped1[,c('ind','mo','fa')])$Ainv
ped1[is.na(ped1)] <- 0
...
### MCMC glmm
		formula <- Y ~ 1 + age + sex + Z + f(idU, model="generic0", Cmatrix=Prec,
								   hyper = list(prec = list(prior = "pc.prec"
															, param = c(3, .01))))
		mINLA <- inla(formula, data = fam_data, family = "binomial", Ntrials = rep(1, n_total), 
					  control.inla = list(correct = T, correct.factor = 10), num.threads = 1)
		var.post <- inla.tmarginal(function(logprec) 1/exp(logprec), mINLA$internal.marginals.hyperpar[[1]])
		a_var_hat <- inla.emarginal(function(x) x, var.post)
		fam_data$U_hat <- mINLA$summary.random$idU$mean
		res_fam <- matrix(crossprod(Prec, fam_data$U_hat)/a_var_hat, n_person, N_fam)
