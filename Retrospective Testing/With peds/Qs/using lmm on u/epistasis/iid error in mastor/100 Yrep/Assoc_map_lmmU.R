...
### MCMC glmm
		...
		data = fam_data, verbose = F, nitt=300e3, thin=500, burnin=25e3, pr = TRUE)
		ck <- ((16*sqrt(3))/(15*pi))^2
		fam_data$U_hat <- posterior.mode(mMCMC$Sol[,-(1:(ncol(fam_data)-1))])/sqrt(1 + ck) #includes intercept
...
M = rbind(c(0,5.2),c(0.6,6.1),c(.8,6.3),c(1,6.4))
n_M = dim(M)[1]
...
for(i in 1:n_M) {
  res[[i]] = mclapply(1:n_rep, runme, mc.cores = 10, Tu= M[i,1], Tot = M[i,2])
