# 200K MCMC iterations W no asc. -- Using GLS-based retro. score statistic
library(mvtnorm)
library(parallel)
library(reshape)
...
runme = function (dummy, Tu, Tot)
{
	...
	beta <- beta * S_XB
	LG <- log(2.5) # increase of 10% in the odds of being a case
	int <- qlogis(0.025) - beta[1] * 31.86 -  beta[2] * 1.5 - LG * mean_G1 # Prevalence - b * E(X) - lambda * mean of epistatis indicator
	...
	while(any(is.na(pvals_t1err)) | any(is.na(pvals_power))){
		...
		for (i_fam in 1:N_fam){
			...
			while(ascert == 0){
				...
				if(sum(Y_f) >= 0) ascert <- 1 #At least 6 affected
			}
			Y <- append(Y, Y_f)
...
## Using GLS to model u against G + g
		res_fam <- as.vector(crossprod(Prec, fam_data$U_hat))
		g_bar <- colMeans(Gmat)
...
M = rbind(c(0,5.2),c(0.2,6.1),c(.8,6.3))
n_M = dim(M)[1]
