# 200K MCMC iterations w ascertainment (6+ in fam)
library(mvtnorm)
library(parallel)
library(reshape)
...
### MCMC glmm
	...
	### Check effective size of VCV samples
	if( (siz1 <- effectiveSize(mMCMC$VCV[,1])) < .2 * 500){
	mMCMC2 <- MCMCglmm(Y ~ 1 + age + sex + Z, random =~idU, family = "categorical",
	ginverse = list(idU = Prec), prior=prior1,
	data = fam_data, verbose = F, nitt=250e3, thin=500, burnin=15e3)
	mMCMC_comb_Sol <- rbind(mMCMC$Sol, mMCMC2$Sol)
	mMCMC_comb_VC <- rbind(mMCMC$VC, mMCMC2$VC)
	siz1 <- siz1 + effectiveSize(mMCMC2$VCV[,1])
	} else {
		mMCMC_comb_Sol <- mMCMC$Sol
		mMCMC_comb_VC <- mMCMC$VC
	}
	ck <- (16*sqrt(3)/(15*pi))^2
	pvalstot <- list(betas = posterior.mode(mMCMC_comb_Sol/sqrt(1 + ck * rowSums(mMCMC_comb_VC))),
		vars = posterior.mode(mMCMC_comb_VC[,1] / (1 + ck)),
		effect_size = siz1)
	return(pvalstot)
}
