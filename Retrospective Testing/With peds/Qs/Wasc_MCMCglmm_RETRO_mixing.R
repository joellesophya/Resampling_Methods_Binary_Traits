# 200K MCMC iterations W diff asc schemes
library(mvtnorm)
library(parallel)
library(reshape)
...
runme = function (dummy, Tu, Tot)
{
	...
	for (i_fam in 1:N_fam){
		...
		while(ascert == 0){
			...
			if(sum(Y_f) > 8) ascert <- 1 #At least 6 affected
		}
		Y <- append(Y, Y_f)
...
### MCMC glmm
	...
	mMCMC_naff9 <- MCMCglmm(Y ~ 1 + age + sex + Z, random =~idU, family = "categorical",
	ginverse = list(idU = Prec), prior=prior1,
	data = fam_data, verbose = F, nitt=250e3, thin=1000, burnin=25e3)
	ck <- (16*sqrt(3)/(15*pi))^2
	pvalstot <- list(betas = posterior.mode(mMCMC$Sol/sqrt(1 + ck * rowSums(mMCMC$VCV))),
		vars = posterior.mode(mMCMC$VCV[,1] / (1 + ck)))
	return(pvalstot)
}
