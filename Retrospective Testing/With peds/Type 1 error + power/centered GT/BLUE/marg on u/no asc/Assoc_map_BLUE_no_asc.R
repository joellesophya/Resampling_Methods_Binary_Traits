# 250K MCMC iterations W no ascertainment
library(mvtnorm)
library(parallel)
library(reshape)
...
### MCMC glmm
		...
		data = fam_data, verbose = F, nitt=250e3, thin=500, burnin=5e3, pr = FALSE)
		pi_hat <- as.vector(predict_MCMC(mMCMC, marginal =~idU, type = "response")) # E(pi)
