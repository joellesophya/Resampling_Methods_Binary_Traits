# Changed:
# Effective sample size threshold of 40 w/ 200K MCMC iterations
...
runme = function (dummy, Tu, Tot)
{
  ...
  while(any(is.na(pvals_power))){
    ...
    mMCMC <- MCMCglmm(Y ~ 1 + age + sex + Z, random =~idU, family = "categorical",
                      ...
                      data = fam_data, verbose = F, nitt=200e3, thin=500, burnin=15e3, pr = TRUE)
	...
	# Get rand. eff. pred.
	EU_hat_mean <- as.vector(crossprod(Prec, colMeans(mMCMC$Sol[,-(1:mMCMC$Fixed$nfl)])) )
    EU_hat_mode <- as.vector(crossprod(Prec, posterior.mode(mMCMC$Sol[,-(1:mMCMC$Fixed$nfl)])) )
	pvals_power <- get_stats(matrix(EU_hat_mean, n_person, N_fam))
    pvals_power <- append(pvals_power, get_stats(matrix(EU_hat_mode, n_person, N_fam)))
    
    if((effS <- effectiveSize(mMCMC$VCV)[1]) < 40){
      mMCMC2 <- MCMCglmm(Y ~ 1 + age + sex + Z, random =~idU, family = "categorical",
                         ginverse = list(idU = Prec), prior=prior1,
                         data = fam_data, verbose = F, nitt=850e3, thin=1000, burnin=15e3, pr = TRUE)
      post_u_mean <- colMeans(rbind(mMCMC$Sol,mMCMC2$Sol)[,-(1:mMCMC$Fixed$nfl)])
      post_u_mode <- posterior.mode(rbind(mMCMC$Sol,mMCMC2$Sol)[,-(1:mMCMC$Fixed$nfl)])
      EU_hat_mean_effS2 <- as.vector(crossprod(Prec, post_u_mean))
      EU_hat_mode_effS2 <- as.vector(crossprod(Prec, post_u_mode))
	  pvals_power <- append(pvals_power, get_stats(matrix(EU_hat_mean_effS2, n_person, N_fam)))
	  pvals_power <- append(pvals_power, get_stats(matrix(EU_hat_mode_effS2, n_person, N_fam)))
	  effS2 <- effS + effectiveSize(mMCMC2$VCV)[1]
    } else {pvals_power <- append(pvals_power, rep(NA,2)); effS2 <- 0}
  ...
  pvalstot <- list(Power = pvals_power, effSizes = c(effS, effS2))
  return(pvalstot)
}
...
plot_power_effsiz <- function(meth_list, level, thr){
	...
	pdf(paste0("power_effSiz_level", level*100,"pct.pdf"))
...
plot_power_effsiz(method_used, level, thrs)