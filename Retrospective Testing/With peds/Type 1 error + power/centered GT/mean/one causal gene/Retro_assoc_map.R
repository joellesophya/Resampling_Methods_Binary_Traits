...
### MCMC glmm
		...
		## get post mean of sig and u
		M4 <- solve(Phimat , colMeans(est_u)/mean(est_s2))
		## get post mean of (sig^-2 * Phi_inv * u)
		M5 <- colMeans(t(solve(Phimat , t(est_u)))/est_s2)
...
# Retrospective denominator
		...
		D5 <- 2 * mafs * (1 - mafs) * (M5 %*% crossprod(Phimat, M5))
		...
		pvalstot <- cbind(pvalstot, pchisq(N5 / D5, df = 1, lower.tail = F))
