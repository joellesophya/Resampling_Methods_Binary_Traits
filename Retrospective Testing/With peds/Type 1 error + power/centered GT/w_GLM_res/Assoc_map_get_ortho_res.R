...
### MCMC glmm
		...
		U_hat <- posterior.mode(mMCMC$Sol[,-(1:4)])/sqrt(1 + ck)
		m_log <- glm(Y ~ 1 + age + sex + Z + offset(U_hat), family = binomial(link = "logit"),
		data = fam_data)
		res_hat <- resid(m_log, type = "response")

## Generating unassociated Gs
		n_G <- 250 # 50 * n_rep replicates for type 1 error
		Gmat <-  cbind(G1, replicate(n_G, rowSums(peddrop( ped1[,c("fa","mo")], 0.3 )))) # G1 is first column + n * n_G matrix 
# Numerator
		T_num_G <- as.vector(crossprod(Gmat, res_hat))^2
...
# Retrospective denominator
		...
		YPhiY <- crossprod(res_hat, Phimat) %*% (res_hat) #marginal mean
...
M = rbind(c(0,5.2),c(.8,6.3),c(1,6.4))
n_M = dim(M)[1]
...
for(i in 1:n_M) {
  res[[i]] = mclapply(1:n_rep, runme, mc.cores = 5, Tu= M[i,1], Tot = M[i,2])
